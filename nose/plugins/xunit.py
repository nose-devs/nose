...
If you want test docstrings in the xml, you can set the
``--xunit-show-docstrings`` option.
...
class Xunit(Plugin):
    ...
    def options(self, parser, env):
        ...
        parser.add_option(
            '--xunit-show-docstrings', dest='show_docstrings',
    ...
    def addSuccess(self, test, capt=None):
        ...
        self.errorlist.append(
            '<%(testcase)s>'
            '%(systemout)s%(systemerr)s</testcase>' %
            {'testcase': testcase,
             'systemout': self._getCapturedStdout(),
             'systemerr': self._getCapturedStderr(),
             })

    def buildTestcaseTag(self, test):
        """Builds up the test case tag for the XML output"""

        id = test.id()
        taken = self._timeTaken()

        if self.show_docstrings:
            if test.test.test.__doc__:
                desc = ' description="%s"' % inspect.cleandoc(test.test.test.__doc__)
            else:
                desc = ' description="MISSING"'
        else:
            desc = ""

        return 'testcase classname=%(cls)s name=%(name)s%(desc)s time="%(taken).3f"' % \
