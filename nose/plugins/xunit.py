...
If you want to include the test's docstring in nosetests.xml, set the
...
class Xunit(Plugin):
    ...
    def addError(self, test, err, capt=None):
        ...
        self.errorlist.append(
            u'<testcase classname=%(cls)s name=%(name)s description="%(desc)s" time="%(taken).3f">'
            u'<%(type)s type=%(errtype)s message=%(message)s><![CDATA[%(tb)s]]>'
            u'</%(type)s>%(systemout)s%(systemerr)s</testcase>' %
            {'cls': self._quoteattr(id_split(id)[0]),
             'name': self._quoteattr(id_split(id)[-1]),
             'desc': test.test.test.__doc__ if self.show_docstrings else '',
             'taken': taken,
             'type': type,
